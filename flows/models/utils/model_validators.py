...
def validate_non_empty_string(value: Any, field_name: str, context: str) -> None:
    ...
    assert isinstance(value, str), (
        f"{context}: field '{field_name}' must be a string; got {type(value).__name__}: {value!r}"
    )
    assert value.strip(), (
...
def validate_model(model: Mapping[str, Any], index: int) -> None:
    ...
    for field_name in ("name", "license", "source", "tags"):
        assert field_name in model, f"{context}: missing required field '{field_name}'"
    ...
    assert isinstance(tags, list), (
        f"{context}: field 'tags' must be a list of objects; got {type(tags).__name__}: {tags!r}"
    )
    ...
    for tag_index, tag in enumerate(tags):
        ...
        assert "name" in tag, f"{context}: missing required field 'tags[{tag_index}].name'"
    ...
    if "quantized_files" in model:
        ...
        for file_index, file_info in enumerate(quantized_files):
            ...
            assert isinstance(file_info, Mapping), (
                f"{file_context}: expected an object, got {type(file_info).__name__}: {file_info!r}"
            )
            size_bytes = file_info.get("size_bytes")
            if size_bytes is not None:
                assert isinstance(size_bytes, (int, float)) and not isinstance(size_bytes, bool), (
                    f"{file_context}: field 'size_bytes' must be numeric, "
                    f"not boolean; got {type(size_bytes).__name__}: {size_bytes!r}"
                )
                assert size_bytes >= 0, (
                    f"{file_context}: field 'size_bytes' must be at least zero; got {size_bytes!r}"
                )
