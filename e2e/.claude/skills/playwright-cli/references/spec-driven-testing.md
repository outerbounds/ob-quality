...
> **Anaconda projects:** the planner / generator / healer agents own plan → generate → heal — prefer them over this generic workflow. If you do follow it directly, write plans to `tests/test-plans/` and translate all captured code per [test-generation.md](test-generation.md) Steps 2–5 (library functions, `@fixture` imports, class-based POM) — never raw `@playwright/test` imports or CLI-generated role/text locators.
...
Loop 2.2 over the targeted scenarios one at a time, restarting the seed between each so every test starts from a clean page. Generate sequentially — each scenario reuses the seed flow, so parallel generation sessions would interfere; the generated tests themselves can run in parallel later.
...
The test is paused at the start. Step forward or run until just before the failing action or assertion, then diagnose:
