...
- **UI vs. API** — classify before exploration using Test Type Routing below. Browser Strategy applies only to UI cases; API cases use endpoint contracts and existing API code.
...
- **An API endpoint, contract, or endpoint implementation** — identify the method, path, expected behavior, and authentication requirements; plan endpoint coverage without requiring a UI.

Classify the scope using Test Type Routing before exploring a URL. For UI cases, use Browser Strategy below. When no environment is reachable, still survey existing coverage and plan from the requirement or contract, marking unverified steps, selectors, or response expectations as provisional.

## Test Type Routing

One planner owns the feature's coverage across both layers. Classify each candidate as **UI** (drives a user interface, including a UI flow that asserts on network traffic) or **API** (exercises HTTP endpoints directly). Record `**Test type:** UI` or `**Test type:** API` on every case; a mixed plan classifies per case.

- **UI cases:** follow browser exploration and locator guidance. For a new UI case, plan its page object, spec, and test data under `tests/pages/ui/`, `tests/specs/ui/`, and `tests/testdata/ui/`; API equivalents belong under their `api/` folders. Preserve an existing project's layout rather than planning a file move.
- **API cases:** inspect available contracts, endpoint implementations, existing API classes/specs, endpoint builders, and project auth configuration. Load the API coverage and test-data references below. Record contract/source evidence, method/path, auth requirements (including public endpoints), request data, expected status/body, and setup/cleanup for mutations. Missing evidence stays provisional; do not invent a response schema or credentials. Live requests are optional for planning and must respect the coverage reference's shared-environment and mutation rules.
- **Coverage decisions:** use Gate 0 to compare actual assertions and failure modes at each layer, reading the relevant spec when an index or title is insufficient. API coverage does not establish that a UI journey works; a UI happy path does not establish endpoint validation or authorization coverage. Keep both when they cover distinct behavior, while avoiding duplicate checks within the same layer.
- **API-only plans:** skip browser opening/closing, snapshots, locator discovery, and UI-only references/checks. Apply Gates 0–2, dispositions, test-data rules, and formatting to both types. Browser/navigation-specific rules in those gates apply only to UI candidates.
...
**Load the references for the case types being planned** — the first four are UI-only; API-only plans skip them. Project skill discovery applies to both types:
...
Every applicable item must pass before you save the plan. Navigation, locator, and browser-action checks apply only to UI cases:

- [ ] Every case declares `**Test type:** UI` or `**Test type:** API`; API coverage and UI journey coverage have been assessed separately.
- [ ] API cases contain the API handoff fields in Output Format, with evidence or provisional gaps, contract-based expectations, and safe setup/cleanup for mutations. Public endpoints do not acquire invented auth requirements.
...
- [ ] **Test data:**
  - Every case that consumes input **or asserts a value** (form input, expected URL/`href`, expected link label, expected text content, display name, header greeting, etc.) names its `**Test data:**` and its source in the project's established test-data layout (existing key reused where present). Paths such as `tests/testdata/users.ts` are valid; do not relocate or duplicate data to satisfy the UI/API defaults. When no layout exists, use `tests/testdata/ui/` or `tests/testdata/api/`. No hardcoded literals in plan steps or in the **Test data:** block — boundary/empty values are named keys too (an empty string is `accountData.emptyName`, not an inline `''`).
...
2. **Explore by Test Type**
   - For API cases, follow Test Type Routing: inspect endpoint evidence and record the API handoff fields. The browser instructions below apply only to UI cases.
...
3. **Close Browser**
   - If you opened a browser, close it immediately after exploration, before writing the plan: `playwright-cli close`

4. **Analyze User Flows**
   - For API cases, analyze request/response behavior, authorization, and data lifecycle using the endpoint coverage reference. Apply the scope budget to distinct contract behaviors; justify necessary overages instead of dropping required coverage.
...
5. **Write the Plan**
   ...
   Each scenario carries the applicable keys in the Output Format template below. UI cases assume a blank/fresh starting state unless the case declares a **Seed:**; API cases declare their prerequisites in **Setup/cleanup:**.
   ...
   **API plan step syntax:** describe the request followed by separate response verification, with proposed or existing API-class action/`verify*` methods in `**Spec calls:**`. Mark proposed methods as new; verify library helper names against documented exports. Expected status/body values come from the contract and named test data. Do not invent locator fields or navigation for API cases.

   **UI plan step syntax** — locator functions find elements, assertion functions verify them; never merge the two:
...
**Target spec:** `tests/specs/{ui|api}/{app}-{feature}.spec.ts` ← file the generator should create OR extend; choose the folder that matches each case's test type. The per-case **Disposition:** is authoritative when cases target different files.
...
**Test type:** {required — UI or API; classify each case even in mixed plans}
...
**Endpoint:** {API only, required — HTTP method + path template and existing endpoint builder to reuse, or the builder to add}

**Contract source:** {API only, required — contract/implementation/spec path or supplied requirement; identify unverified expectations as provisional}

**Authentication:** {API only, required — public, or required auth/role and existing fixture/config source; never include credential values}

**Request:** {API only, required — query/path/body test-data keys and relevant header names; state none when absent}

**Response:** {API only, required — expected status and body/schema/side-effect checks grounded in the contract; name existing constants/test-data keys where applicable}

**Setup/cleanup:** {API only, required — prerequisites, per-case data ownership, setup, and cleanup even after assertion failure; state not applicable only when no setup or cleanup is needed, including for read-only cases}

**Spec calls:** {required for API cases — API-class action then response-verification methods, marking proposed methods as new; optional for UI cases — when multiple page-object methods run in one test(), list them in order; for navigation cases use action then verify — e.g. `clickCategoryATile(), verifyCategoryAPage()`}
...
1. {UI: action or assertion using documented @anaconda/playwright-utils functions and verified locators. API: request action then separate response verification using the API-class methods in Spec calls and the named request/response data.}
2. ...
...
Worked examples and mechanics for the rules above live in the planner anti-patterns reference. Use the active agent-tree path: in repo-checkout/eval mode, `templates/agents/references/planner-anti-patterns.md`; in an installed consumer agent, `.claude/agents/references/planner-anti-patterns.md` — **including when this agent definition itself loaded from `.kilo/agents/`**, since reference docs are always read from the `.claude/agents/references/` tree, never a `.kilo/agents/references/` equivalent. Load it when shaping cases. **Blocking precondition:** when any Gate 1 candidate is a link/tile/card/chip, or any core eval returned `dupCount > 1`, Read the relevant appendix from that active path before running Gate 2.
