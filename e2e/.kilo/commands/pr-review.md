...
3. In **one Bash tool call**, run the post, capture its status, remove the body file, and return the captured status:
   ...
   ```bash
   gh pr comment "<PR>" --body-file "<temporary body-file path>"; status=$?; rm -f "<temporary body-file path>"; exit "$status"
   ```

Never interpolate the report into a shell argument (for example, `--body "<body>"`) or a heredoc. Never post in local or base-ref mode, never post without the keyword.

## Hard rules

- Never run `git add`, `git commit`, `git push`, `git tag`, `git stash`, `git restore`, `git checkout`, or `gh pr checkout`. The working tree must be byte-identical after the run; the explicitly allowed temporary PR body file is outside it.
