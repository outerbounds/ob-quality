...
- **Default in-scope set** (used by sections 1, 3, 4): `CLAUDE.md`, `AGENTS.md`, `.claude/skills/**`, `.claude/agents/**`, `.kilo/agents/**`, `.kilo/instructions/**`, `.claude/commands/*.md`, `.kilo/commands/*.md`. `.kilo/kilo.jsonc` is explicitly excluded because it is a create-once user config.
...
**Root-doc preflight (before either setup pass).** If the active in-scope set contains `CLAUDE.md` or `AGENTS.md`, each such file must already exist. If either is missing, **stop before section 2**: `anaconda-pw-setup --keep-local` can prompt for a missing root doc, while `--overwrite-all` can create one after the recovery baseline was staged. Ask the user to resolve root-doc setup in an interactive terminal (for example, `npx anaconda-pw-setup --docs`), then rerun this command; alternatively, they may intentionally remove that root-doc path from their profile's scope. Never add `--overwrite-docs` to this workflow merely to bypass the check, because it can overwrite a customized existing root doc.
...
**Reload reconciliation scope before classifying or staging.** When `managePackageOwnedScope` is `true`, `anaconda-pw-setup --keep-local` can backfill missing package-managed entries in a valid `.claude/reconcile-ai-profile.json`. Re-read and validate the profile exactly as in section 0, then recompute the in-scope AI paths from its current `projectFacingAiFiles` and `libraryOwnedAiFiles` values. Use this refreshed scope for the rest of sections 2–4, including the baseline, staging list, and completeness gate. Do not stage the profile itself: it remains a project-owned configuration file and `--overwrite-all` does not replace it.
