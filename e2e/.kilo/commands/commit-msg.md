...
2. **Staged plus unstaged or untracked** — staged changes exist AND (unstaged changes exist OR untracked files exist) → ask the user "What should I describe?" with two options: `Staged changes only` and `Everything (staged + unstaged + untracked)`, using the assistant's user-choice tool (Claude Code's AskUserQuestion tool, Kilo's question tool). Wait for the selection before continuing.
