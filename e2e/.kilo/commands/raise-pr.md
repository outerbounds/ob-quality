...
```bash
gh pr view --json number,url,title,body,headRefName,headRefOid,baseRefName
```

If a PR URL was provided after `update`, run:

```bash
gh pr view "<pr-url>" --json number,url,title,body,headRefName,headRefOid,baseRefName
```
...
Run `git rev-parse HEAD` as a separate Bash tool call. If its SHA does not exactly match the resolved PR `headRefOid`, stop before gathering the diff or running `gh pr edit` and say: `Your local HEAD does not match the current PR head. Fetch or integrate the PR branch, or push your local commits as appropriate, then re-run /raise-pr update so the PR and its description stay in sync.` Do not fetch, merge, rebase, or push on the user's behalf. This check is required even when the branch has no upstream.
...
subshells — those compound shell forms trigger the assistant's "cannot be
statically analyzed" approval prompt (Claude Code and Kilo both gate compound
shell forms this way).
